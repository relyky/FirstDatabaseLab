...
## EF Core Tools 管理目錄
目標專案下。需要透過 appsettings.json 拿到連線字串。若不是啟動專案則用參數`--startup-project`指定。   
另有`Migrations/`目錄紀錄『schema 差異指令』。這很重要應該與真實異動相符。   
還有`Schema/`目錄由參數`--output-dir`指定。
```
Project/
  appsettings.json ` --startup-project 取連線字串
  Migrations/
    yyyyMMddHHmmss_name1.cs
    yyyyMMddHHmmss_name2.cs
